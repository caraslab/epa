...
validTimeRange = [ceil(validTimeRange(1,:)/2.56e-6) * 2.56e-6;floor(validTimeRange(2,:)/2.56e-6) * 2.56e-6];
...
for ev = 1:numel(eventNames)
    ...
    for jjj = hour_values
        ...
        missing = sum(diff(file_list_temp(temp_num).gaps));
    ...
    if ~all(cellfun(@isempty, {file_list_temp.gap_text}))
        data.(eventName).data = [];
    ...
    for ii = 1:numRanges
        ...
        for chan = channels
            chanIndex = 1 + this_start_sample - 1;
